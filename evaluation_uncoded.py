...
def main():
    ...
    parser.add_argument("-c", "--config_file", help="configuration file", type=str, default='config_eval_uncoded.yml')
    ...
    if bits_info<15:
        ...
        all_words = all_info_words.clone()
    ...
    # ---- Declare model ---- #
    model = CodedVAE(enc, dec, bits_info, likelihood=likelihood, beta=15, lr=1e-4, inference=inf_type, seed=0, device=device)
    ...
    if bits_info < 15:
        with torch.no_grad():
            ...
            if inf_type =='bit':
                entropy_raw = entropy_raw/len(testloader)
    ...
    with torch.no_grad():
        ...
        word_sample = torch.bernoulli(bit_probs)
        z_sample  = modulate_words(word_sample, beta=torch.tensor(15.))

        # 2. Forward the sampled words through the decoder to obtain reconstructed images
        decoder_out =  model.decoder.forward(z_sample.to(model.device))

        # 3. Forward the reconstructed image through the encoder to obtain the recovered word
        bit_probs_recovered = model.encoder.forward(decoder_out)
        word_recovered = torch.bernoulli(bit_probs_recovered)
        # 4. Compare the original sampled word with the recovered word
        ber = (word_sample != word_recovered.cpu().data).sum()/(word_sample.shape[0]*bits_info)     # Bit Error Rate = n_bit_errors/n_bits_transmitted
        wer = ((word_sample != word_recovered.cpu().data).sum(dim=1) != 0).sum()/word_sample.shape[0]   # Word Error Rate = n_words_incorrect/n_words_transmitted
    ...
    with torch.no_grad():
        if dataset == 'FMNIST' or dataset=='MNIST':
            ...
            if bits_info<15:
                ...
                hit_score_test_conf = eval_reconstruction(classifier_network, model, testloader,code_words=all_words, threshold=0.4)
